#!/usr/bin/env python3
"""
build-comics.py v1.2.1 - Image Viewer Generator
...
def find_images(folder):
    """Find all images in folder, sorted naturally"""
    images = []
...
def parse_toc(folder):
    ...
    for line in toc_file.read_text(encoding='utf-8').splitlines():
        ...
        if parts[0].upper() == 'TOPIC' and len(parts) >= 3:
            ...
            # Page = topic number (01 -> page 1, etc)
            try:
                page = int(topic_num)
            except:
                page = 1
...
def get_template():
    ...
    return '''<!DOCTYPE html>
<!-- build-comics.py v__VERSION__ | __DATE__ -->
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <meta name="generator" content="build-comics.py v__VERSION__">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .image-container.fit-width img { width: 100%; height: auto; }
        ...
        .side-nav {
            ...
            background: rgba(255,255,255,0.9); color: #0066cc; border: none;
            width: 36px; height: 36px; border-radius: 50%;
            cursor: pointer; opacity: 0.4; transition: opacity 0.2s; font-size: 16px;
        }
...
function toggleTOC(show) {
    document.body.classList.toggle('toc-hidden', show === false ? true : show === true ? false : undefined);
}
...
document.addEventListener('keydown', e => {
    ...
    if (state.zoom > 1) {
        if (e.key === 'ArrowLeft') { pan(40, 0); e.preventDefault(); return; }
        if (e.key === 'ArrowRight') { pan(-40, 0); e.preventDefault(); return; }
        if (e.key === 'ArrowUp') { pan(0, 40); e.preventDefault(); return; }
        if (e.key === 'ArrowDown') { pan(0, -40); e.preventDefault(); return; }
    }
    if (e.key === 'ArrowLeft') loadPage(state.page - 1);
    else if (e.key === 'ArrowRight') loadPage(state.page + 1);
...
def build(folder):
    ...
    print(f"build-comics.py v{VERSION}")
