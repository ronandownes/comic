#!/usr/bin/env python3
"""
MATHS COMIC INDEX BUILDER
=========================
version: v004
date: 2025-02-02

Scans existing WebPs (TTPP.webp) and generates index.html with strand TOC.
"""
...
def scan_webps(folder):
    ...
    for f in folder.glob("*.webp"):
        # Match TTPP or TPP format (1-2 digit topic, 2 digit page)
        m = re.match(r'^(\d{1,2})(\d{2})\.webp$', f.name)
        if m:
            topic = int(m.group(1))
            page = int(m.group(2))
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .topic-name {{ font-size: 15px; color: #fff; }}
        ...
        .image-container img {{ width: 100%; max-width: 950px; height: auto; }}
...
function updatePage() {{
    const img = currentTopic + String(currentPage).padStart(2,'0') + '.webp';
...
def main():
    print("=" * 50)
    print("MATHS COMIC INDEX BUILDER v004")
    print("=" * 50)
    ...
    # Scan existing WebPs
    page_counts = scan_webps(COMIC_PATH)
